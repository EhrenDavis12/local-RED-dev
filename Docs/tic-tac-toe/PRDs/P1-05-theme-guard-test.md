...
`P1-03-theme-system.md`: the baseline starts empty, so the guard is meaningful against an
empty `lib/` and stays meaningful as code lands.
...
`Color(0xFF00E5FF)` or `Duration(milliseconds: 200)` typed into a widget is invisible at review
time, works fine under Neon, and only surfaces as a bug when Classic Red vs Blue is selected and
one thing on screen refuses to change. Since there is no application code yet
([Tech Design](../Tech%20Design.md) → Decisions → Fresh build, not a refactor), the cost of
holding the line is zero today and grows with every file added.
...
4. **All six categories from the Architectural Rule are covered.** Colors, backgrounds, fonts,
   piece styles, sounds, and animations each have at least one detection rule. A guard covering
   five of the six does not satisfy this PRD.
   *Source:* [Tech Design](../Tech%20Design.md) → Decisions → Do we add a test that fails on
   hardcoded theme values? (*"covering all six categories the Architectural Rule names"*);
   [Theming](../Theming.md) → Architectural Rule (the six-item list).
   *Verification:* for each of the six categories, introducing a representative violation into a
   scanned file makes the suite fail, and removing it makes the suite pass.
...
6. **The concrete pattern set is sharpened during implementation, not frozen here.** Tech Design
   ...
   board widgets; literal `assets/…` paths outside the theme layer — and states plainly that
   these are *"to be sharpened at the keyboard rather than settled here."* This PRD carries that
   framing: the table is the starting point and the obligation is Requirement 4 (all six
   categories), not any particular regex.
   *Source:* same Decision, indicative-patterns table and the sentence introducing it.
...
- **The theme system itself** — the theme object, the YAML loader, merge-over-Neon,
  materialization. See `P1-03-theme-system.md`. This PRD delivers the guard, not the thing
  being guarded.
- **The rules-engine unit tests** (`P1-02-engine-rules.md`) and the **board widget tests**
  (`P2-01-board-rendering.md`). Three separate test decisions sit side by side in
...
2. **The exact patterns per category.** Deliberately deferred by
   ...
   here."* Recorded as deferred, not resolved.
...
6. *(Raised by this PRD — a scope tension in the source material.)* **Are chrome icons theme
   ...
   "piece styles," which is the category [Theming](../Theming.md) → Architectural Rule actually
