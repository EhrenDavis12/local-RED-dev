...
if [ "$n_imports" -eq 0 ]; then
  alert "CLAUDE.md names no active system. Expected exactly one line matching @.claude/systems/<name>/SYSTEM.md — no system rules are loaded. Run /set-system."
elif [ "$n_imports" -gt 1 ]; then
  alert "CLAUDE.md has $n_imports system import lines; exactly one is allowed. Two systems' rules are in context at once, which is the thing this design exists to prevent. Run /set-system to fix."
else
  ...
  if [ ! -f "$target" ]; then
    ...
    alert "CLAUDE.md imports $target but that file does not exist. The active system's rules are missing from this session. Run /set-system to repoint it."
    system=""
  fi
fi
...
if [ -n "$system" ] && [ -f "$SETTINGS" ] && compgen -G ".claude/systems/*/system.json" >/dev/null; then
  ...
  if [ "$want_deny" != "$have_deny" ]; then
    alert "Half-finished system swap: with \"$system\" active, settings.json should deny [${want_deny:-none}] but denies [${have_deny:-none}]. Agents from an inactive system are still dispatchable. Re-run /set-system $system."
  fi
  if [ "$want_skills" != "$have_skills" ]; then
    alert "Half-finished system swap: with \"$system\" active, settings.json should switch off [${want_skills:-none}] but switches off [${have_skills:-none}]. Re-run /set-system $system."
  fi
fi
...
if [ -n "$system" ]; then
  ...
  sys_line="Active system: $system — $title. Its rules are imported at the end of CLAUDE.md. To swap, run /set-system."$'\n'
fi
...
if [ ! -f "$POINTER" ]; then
  finish "No active project is set. Agents that resolve their scope from the manifest will refuse to run until one is. Run /set-project."
fi

slug=$(jq -r '.project // empty' "$POINTER" 2>/dev/null) || slug=""
if [ -z "$slug" ]; then
  finish "$POINTER exists but names no project. Run /set-project to set one."
fi
...
if [ -z "$manifest" ]; then
  finish "The active project is \"$slug\" but no Docs/*/project.json declares that name. The manifest is missing or its .name does not match. Run /set-project to fix this before dispatching any agent."
fi
