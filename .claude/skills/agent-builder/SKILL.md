...
| Dissolving one doc's `## Decisions` orphaned **nine references to it from three other docs** — the plan changed a heading without looking for who pointed at it | A plan that moves, renames, or dissolves a heading must grep for its callers and repoint them **in the same plan**. A follow-up run leaves the docs knowingly broken in between, and that state gets committed. Generally: **when a change alters a name others use, the plan owns the callers too** |
| Every agent shipped with `maxTurns: 80` copied from the first file; 60 of 381 logged runs blew past their caps — 73% of code-writer runs, one at 3.8× — and none was stopped. The caps also sat *below* the observed medians, so had they ever fired they would have killed the majority of real work | A guard nobody has seen fire is not a guard. Verify an enforcement mechanism fires once before relying on it, and derive its threshold from the metrics log, never by copying the last agent's |
| The metrics README carried a "drowning in context" diagnostic with 17 triggering runs in the log, and a cap-sizing procedure never once applied — rules with no one assigned to run them | A report nobody runs is a rule that cannot fire. Every diagnostic names its consumer; `forge-retro-planner` is the roster's |
